...
*ad_text.sh*)
    for clip in `find /bak -name "*.mp4" -print`; do
        ...
        if test ! -f "/archive/$clip_name"; then
            ffmpeg -i "/bak/$clip_name" -vf "scale=1920:960,pad=1920:1080:0:60:blue,drawtext=text='Server-Side AD Insertion':x=(w-text_w)/2:y=30:fontsize=30:fontcolor=white" -y "/archive/$clip_name"
        fi
    done
    wait
    ;;
