...
def parse_hls(stream_cp_url, m3u8, stream_info, ad_spec, ad_segment=5.0):
    ...
    for i in range(len(lines)):
        ...
        if lines[i].startswith("#EXTINF:") and i+1<len(lines) and i+2*(ad_interval+1)+1<len(lines):
            ...
            if segsplayed == 0 and ad_sequence == 0:
                #if ad_sequence == 0:ahead_analytic=ad_interval-1
