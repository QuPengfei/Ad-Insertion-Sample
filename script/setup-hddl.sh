...
#openvino_name="l_openvino_toolkit_p_2019.2.275"
#openvino_name="l_openvino_toolkit_p_2019.2.242"
openvino_name="l_openvino_toolkit_p_2019.1.144"
#ov_link="http://registrationcenter-download.intel.com/akdlm/irc_nas/15944/$openvino_name.tgz"
#ov_link="http://registrationcenter-download.intel.com/akdlm/irc_nas/15693/l_openvino_toolkit_p_2019.2.242.tgz"
#ov_link="http://registrationcenter-download.intel.com/akdlm/irc_nas/15792/l_openvino_toolkit_p_2019.2.275.tgz"
ov_link="http://registrationcenter-download.intel.com/akdlm/irc_nas/15512/l_openvino_toolkit_p_2019.1.144.tgz"
py_ver='3.7.2'
...
{
	gen_Dockerfile
	docker build -t ${ov_hddl}:${ov_hddl_version} ${DIR} $(env | grep -E '(proxy|REPO|VER)=' | sed 's/^/--build-arg /')
...
install_hddl_package_local()
{
        apt-get update && apt-get install -y -q apt-utils lsb-release vim net-tools bzip2 wget curl git gcc g++ automake libtool pkg-config autoconf make cmake cmake-curses-gui gcc-multilib g++-multilib libboost-dev libssl-dev build-essential
        apt-get install -y build-essential libncursesw5-dev libgdbm-dev libc6-dev zlib1g-dev libsqlite3-dev tk-dev libssl-dev openssl libffi-dev libelf-dev
        mkdir -p /root/package
        mkdir -p /opt/intel/
        wget -P /root/package $ov_link
